...
exports.getSaleorAmountFromAdyenInteger = exports.getAdyenIntegerAmountFromSaleor = void 0;
const getDecimalsForAdyen = (currency) => {
  if (currency.length !== 3) {
    throw new Error(`currency needs to be a 3-letter code`);
  }
  const adyenCurrency = adyenCurrencies[currency.toUpperCase()];
...
const getAdyenIntegerAmountFromSaleor = (major, currency) => {
  const decimals = getDecimalsForAdyen(currency);
  const multiplier = 10 ** decimals;
  return Number.parseInt((major * multiplier).toFixed(0), 10);
};
exports.getAdyenIntegerAmountFromSaleor = getAdyenIntegerAmountFromSaleor;
// Some payment methods expect the amount to be in cents (integers)
// Saleor provides and expects the amount to be in dollars (decimal format / floats)
const getSaleorAmountFromAdyenInteger = (minor, currency) => {
  const decimals = getDecimalsForAdyen(currency);
  const multiplier = 10 ** decimals;
  return Number.parseFloat((minor / multiplier).toFixed(decimals));
};
exports.getSaleorAmountFromAdyenInteger = getSaleorAmountFromAdyenInteger;
